## Authors Note
EDIT: I was going fast and missed that Claude cheaped out and went with Option A below. What we really want is Option B, so I cajoled Claude into creating it. Herein lies a tale, because Option B is the "hard" option for any ML system. It requires byte-level accuracy, and that is not a thing that ML models do well, so in the readme in the OptionB folder you'll see a neat table that predicts this will only work about 60% - 85% of the time. So much for AGI. Original note contents continue below -- (pun intended...)
